...
describe "User sees all items associated with given category" do
  it "all items in that category appear" do
    ...
    expect(page).to_not have_content("Tiny Sportscar")
    expect(page).to_not have_content("Tired of having a hard time finding parking downtown but still want to look cool in a sportscar? look no further.")
    expect(page).to_not have_content(299.99)
  end
end
